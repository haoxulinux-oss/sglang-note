...
## 五 什么是 fused / kernel fusion(融合)

下一节的标题里有「fused QKV」,§七 是「fused QKV 收益小结」。这里先把 **fused** 这个词讲清楚。

### 5.1 一句话定义

> **fused = 把"原本要分多次启动的 GPU kernel"合并成"一次启动的 GPU kernel"**;数学结果一致,但显存读写、kernel launch 开销都减少了。

### 5.2 为什么要 fuse:GPU 的两个昂贵开销

跑一个 GPU 操作,有两块成本:

| 开销 | 一次值 | 累计起来 |
|---|---|---|
| **kernel launch**(CPU 通过 driver 通知 GPU 「跑这个 kernel」) | 5–10 μs | 推理一层 transformer 大约 30+ kernel,80 层就是 2400+ 次 launch |
| **显存(HBM)读写** | 每个 token 的 hidden 张量几 KB,KV cache 一层几 MB | 同一份数据被反复读写,带宽爆炸 |

LLM 推理是**显存带宽受限**(memory-bound)的工作负载——尤其 decode 阶段,每个 token 算的事不多,**主要时间花在"把数据从 HBM 读进来再写回去"**。fusion 就是省这两块成本的标准手段。

### 5.3 一个具体例子:fused add + RMSNorm

**朴素写法**(两次 kernel launch,中间张量来回读写):

```python
y    = x + residual          # kernel #1:读 x、读 residual、写 y
out  = rmsnorm(y)            # kernel #2:读 y、写 out
```

数据流:
```
HBM:  [x][residual]  ──读──→ kernel #1  ──写──→ [y]  ──读──→ kernel #2  ──写──→ [out]
                              ↑                        ↑
                           launch 1                 launch 2
```

显存读写次数:**5 次**(读 x、读 residual、写 y、读 y、写 out)。

**fused 写法**(`fused_add_rmsnorm`,一次 kernel):

```python
out  = fused_add_rmsnorm(x, residual)
# 内部:tmp = x + residual; out = rmsnorm(tmp); 两步一气呵成
```

数据流:
```
HBM:  [x][residual]  ──读──→ kernel(内部完成 add + rmsnorm)  ──写──→ [out]
                              ↑
                           launch 1
```

显存读写次数:**3 次**(读 x、读 residual、写 out)。**节省 40% 显存带宽**,launch 开销减半。

> 具体到 RMSNorm 的实现,见 [RMSNorm 与残差透传](llama-rmsnorm-and-residual.zh.md)。

### 5.4 fusion 的两种主要形式

**横向 fusion(operator fusion)**——把两个串行的 op 合并:

```
op1 → op2  ⇒  fused_op12
```

例子:`add + rmsnorm`、`silu + mul`(SwiGLU 的 SiluAndMul)、`gelu_tanh + mul`、`rope + qkv_split`

**纵向 fusion(GEMM fusion)**——把多个独立的 GEMM 合并成一个大 GEMM:

```
y1 = x @ W1
y2 = x @ W2          ⇒    [y1, y2, y3] = x @ [W1, W2, W3]
y3 = x @ W3                     (一次 GEMM)
```

例子:**QKV fusion**(本文重点,W_Q + W_K + W_V → W_QKV)、**gate_up fusion**(W_gate + W_up → W_gate_up,在 MLP 里)。

### 5.5 LLM 推理里常见的 fused 操作清单

| fused 操作 | 是哪两个/几个 op 合的 | 在哪用 |
|---|---|---|
| `fused_add_rmsnorm` | x + residual + RMSNorm | 每层 decoder 入口、attention 后 |
| **`qkv_proj`(本文主角)** | **W_Q · x、W_K · x、W_V · x 三次 GEMM** | self-attention 入口 |
| `gate_up_proj` | W_gate · x、W_up · x 两次 GEMM | MLP 入口 |
| `SiluAndMul` | SiLU(gate) · up | MLP 中间 |
| `flash_attention` | Q·Kᵀ + softmax + ·V + masking | attention 核心 |
| `rope` | (cos/sin 查表) + (q,k 旋转) | attention prepare |
| `sampling`(top-k/top-p) | 排序 + 阈值 + 归一化 + 采样 | 输出层 |

可以看到 SGLang(以及任何高性能推理引擎)**整条 forward 流水都是 fused op 串成的**——朴素 PyTorch 写法跑出来可能慢 2-5 倍,差距全在 fusion。

### 5.6 fused 不是免费的午餐

每个 fused kernel 都是**手写的 CUDA / Triton kernel**(在 `sgl-kernel/` 里),写起来比 PyTorch op 难得多:

- 要手动管理 shared memory 和寄存器
- 要处理各种 dtype / shape / 边界情况
- 一旦 fuse 了就不容易调试和分析(整个块是黑盒)

所以 fusion 选哪些 op 合并是个**工程决策**——只有"经常一起出现 + 数据量大 + 现有性能瓶颈" 的组合才值得 fuse。**fused QKV 就是典型的「值得做」的 fusion**:三个 GEMM 永远一起出现、数据量大、QKV 是 attention 入口的性能瓶颈。

---

## 六 Q、K、V 在代码层面到底怎么算出来
...
### 6.5 权重加载:HuggingFace → SGLang 的拼接
...
## 九 完整初始化代码逐行讲解(`llama.py:139-173`)
...
## 十 split 在 `forward_prepare_native` 里的实际形态
...
`split` **不复制内存**(详见 §6.3)。后面 RoPE 会**原地修改 q、k**(数值变,形状不变),v 全程不动。
