...
预取是**非阻塞**的——`_add_request_to_queue` 调一次就返回,但加载不一定完成。整条链路涉及三个步骤,分布在主循环不同位置:

| 步骤 | 位置 | 作用 |
|---|---|---|
| **① 发起**:`prefetch_from_storage` | `_add_request_to_queue` 入队时 | 异步触发 L3 storage → host RAM → GPU 的拉取(只是把任务投递给 backend 后台线程) |
| **② 收割事件**:`check_hicache_events` | `get_new_batch_prefill` 顶部的「② HiCache 事件检查」 | 主循环里**主动 poll** backend 几个内部队列(prefetch_revoke / backup ack / host_mem_release / loading_check / writing_check),把后台线程报回的完成事件消化掉,推进 host↔GPU 的加载状态 |
| **③ 检查完成**:`check_prefetch_progress` | `get_new_batch_prefill` 主循环里逐请求检查 | 看这个 req 的预取是否进展到可用状态;完成则 `pop_prefetch_loaded_tokens` 拿到 storage_hit_length,可以加入 batch |

为什么需要 ② 这一步?因为 backend 线程跑完之后只是往**控制队列**里塞一条「我完成了」的消息——不会主动通知主循环。`check_hicache_events` 的角色就是「**周期性把这些队列消费掉**」,推进每个 prefetch operation 的状态机。如果跳过这一步,后台明明跑完了,但 `check_prefetch_progress` 永远看不到「已完成」,请求会被卡在队列里。

详见 [`get_new_batch_prefill()` 解析](scheduler-get-new-batch-prefill.zh.md) 中 §三 的②⑤说明。
